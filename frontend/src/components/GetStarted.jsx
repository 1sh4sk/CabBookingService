export default function HowItWorks() {
    ...
    return (
      ...
      {steps.map((step, index) => (
        ...
        >
          {/* Step Number */}
          <div className="relative bg-yellow-500 text-white w-16 h-14 mx-auto bottom- flex items-center justify-center text-lg font-bold rounded-lg">
            <span>{step.id}</span>
            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-gray-200  w-6 h-6 rounded-full"></div>
          </div>

          {/* Step Content */}
          <h3 className="font-semibold text-lg md:text-xl mt-4">{step.title}</h3>
          <p className="text-gray-500 mt-3 text-sm md:text-base">{step.description}</p>

          {/* Connector Line (for larger screens) */}
          {index < steps.length - 1 && (
            <div className="hidden md:block absolute top-0 right-[-80px] transform -translate-y-1/2 border-t-2 border-gray-400 w-10 md:w-14 lg:w-80"></div>
          )}
        </div>
      ))}
    </div>
