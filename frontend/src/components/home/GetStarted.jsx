...
export default function HowItWorks() {
  ...
  return (
    <div className="text-center py-12 px-4 md:px-6 lg:px-20 bg-white">
      {/* Heading */}
      ...
      {/* Steps Container */}
      <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12 lg:gap-4">
        {steps.map((step, index) => (
          <div
            key={step.id}
            className="bg-[#f8f6f1] rounded-lg shadow-md p-6 w-full sm:w-80 md:w-72 lg:w-64 xl:w-80 text-center relative"
          >
            {/* Connector Line */}
            {index < steps.length - 1 && (
              <div className="hidden lg:block absolute top-1/2 left-full transform -translate-y-1/2">
                <div className="border-t-4 border-gray-300 w-20 lg:w-28 xl:w-32 rounded-full"></div>
              </div>
            )}
