...
import { deleteCaptain, getCaptainApproval, getPendingCaptains } from "../../api/adminApi";
import { toast } from "react-toastify";

const CaptainApprovals = () => {

    const [pendingApprovalCaptains, setPendingApprovalCaptains] = useState([]);
    const [loading, setLoading] = useState(true);
    const [approving, setApproving] = useState(false);
    const [rejecting, setRejecting] = useState(false);

    useEffect(() => {
        const fetchPendingApprovalCaptains = async () => {
            try {
                setLoading(true);
                const res = await getPendingCaptains();
                setPendingApprovalCaptains(res.data.pendingcaptains);
            } catch (error) {
                console.error("Error fetching captains with pending approvals:", error);
                toast.error("Failed to load captains with pending approvals.");
            } finally {
                setLoading(false);
            }
        };

        fetchPendingApprovalCaptains();
    }, []);

    const handleCaptainApproval = async (id) => {
        try {
            setApproving(true);
            await getCaptainApproval(id);
            toast.success("captain approved successfully!")
            setPendingApprovalCaptains((prevCaptains) => prevCaptains.filter(captain => captain._id !== id));
        } catch (error) {
    ...
    return (
        <div className="w-[90%] xl:w-full h-full p-4">
            <h1 className="text-2xl font-bold text-yellow-500">Pending Captain Approvals</h1>
            <p className="text-gray-600">Total Captain with pending approvals: {pendingApprovalCaptains.length}</p>
                ...
                <div className="w-full h-full overflow-scroll mt-4">
                    <div className="mt-4  rounded-lg border border-yellow-500 overflow-x-scroll">
                        <table className="w-full min-w-[1000px]">
                            ...
                            <tbody>
                                {pendingApprovalCaptains.length > 0 ? (
                                    pendingApprovalCaptains.map((captain, index) => (
                                        <tr key={captain._id} className="text-center border-b last:border-b-0 border-yellow-500">
                                            ...
                                            <td className="p-2">
                                                <button
                                                    onClick={() => handleCaptainApproval(captain._id)}
                                                    disabled={approving}
                                                    className="bg-green-500 text-white hover:text-green-700  rounded-lg text-xs p-2 flex items-center justify-center gap-1 w-full"
                                                >
                                                    <FontAwesomeIcon icon={faCheck} /> {approving ? "Approving.." : "Approve"}
                                                </button>
                                                <button
                                                    disabled={rejecting}
                                                    onClick={() => handleDeleteCaptain(captain._id)}
                                                    className="bg-red-500 text-white rounded-lg text-xs p-2 hover:text-red-700 flex items-center justify-center gap-1 w-full mt-2"
                                                >
                                                    <FontAwesomeIcon icon={faTimes} /> {rejecting ? "Rejecting..." : "Reject"}
                                                </button>
                                            </td>
                                        </tr>
                                    ))
